...
public class _13_lowestCommonAncestorInGenricTree {
    ...
    public static int lca(Node node, int d1, int d2) {
        // write your code here
        ArrayList<Integer> ll1 = nodeToRootPath(node, d1);
        ArrayList<Integer> ll2 = nodeToRootPath(node, d2);

        int prev = -1;
        int i = ll1.size() - 1;
        int j = ll2.size() - 1;

        while (i >= 0 && j >= 0 && ll1.get(i) == ll2.get(j)) {
            prev = ll1.get(i);
            i--;
            j--;
        }
