...
public class _12_diameterOfABinaryTree {
    ...
    public static int[] diameter2(Node node) {
        ...
        if (l[0] == -1 || r[0] == -1)
            return new int[] { Math.max(l[0], r[0]) + 1, Math.max(l[1], r[1]) };

        return new int[] { Math.max(l[0], r[0]) + 1, Math.max(Math.max(l[1], r[1]), l[0] + r[0] + 2) };
    }
