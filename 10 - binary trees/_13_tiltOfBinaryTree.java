...
public class _13_tiltOfBinaryTree {
    ...
    public static Pair_ tilt_(Node node) {
        // write your code here to set the tilt data member
        Pair_ ans = new Pair_();
        if (node == null)
            return ans;
        Pair_ l = tilt_(node.left);
        Pair_ r = tilt_(node.right);

        ans.tilt += Math.abs(l.sum - r.sum) + l.tilt + r.tilt;
        ans.sum += l.sum + r.sum + node.data;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(tilt_(root).tilt);
    }
