...
public class _07_targetSumPairInBST {
    ...
    private static void pairSum(Node node, int sum, Node root) {
        if (node == null)
            return;
        pairSum(node.left, sum, root);
        if (sum - node.data > node.data) {
            if (find(root, sum - node.data))
                System.out.println(node.data + " " + (sum - node.data));
        } else
            return;
        pairSum(node.right, sum, root);
    }
